...
class KthLargest:
    ...
    def _init_(self, k: int, nums: list[int]):
        
        self.minHeap = nums
        self.k = k
        
        heapq.heapify(self.minHeap) # transform list into minHeap
        
        while len(self.minHeap) > k: # delete extra elements
            heapq.heappop(self.minHeap)
            
            
    def add(self, val: int) -> int:
        
        heapq.heappush(self.minHeap, val) # Add element
        
        if len(self.minHeap) > self.k:  # Maintain k size
            heapq.heappop(self.minHeap)
            
        return self.minHeap[0] # Return Kth largest element
